...
BOOST_AUTO_TEST_CASE(expected_recover)
{
  ...
  BOOST_CHECK_EQUAL(fun(true).bind(add_five).value(), 10);
  ...
  BOOST_CHECK_EQUAL(fun(false).catch_error(recover_error_failure).bind(add_five).valid(), false);
  BOOST_CHECK_EQUAL(fun(false).bind(add_five).catch_error(recover_error_failure).bind(add_five).valid(), false);
