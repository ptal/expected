...
namespace boost
{
  ...
  public:
    ...
    BOOST_FORCEINLINE BOOST_EXPECTED_CONSTEXPR explicit unexpected_type(ErrorType const& e) :
      error_(e)
    {
    }
    BOOST_FORCEINLINE BOOST_EXPECTED_CONSTEXPR explicit unexpected_type(ErrorType&& e) :
      error_(std::move(e))
    {
    }
  ...
  template <class E>
  BOOST_FORCEINLINE BOOST_EXPECTED_CONSTEXPR unexpected_type<decay_t<E> > make_unexpected(E&& ex)
  {
  ...
  template <class E>
  BOOST_EXPECTED_CONSTEXPR bool operator==(const unexpected_type<E>& x, const unexpected_type<E>& y)
  {
  ...
  inline BOOST_EXPECTED_CONSTEXPR bool operator<(const unexpected_type<std::exception_ptr>& x, const unexpected_type<std::exception_ptr>& y)
  {
    return false;
  }
  inline BOOST_EXPECTED_CONSTEXPR bool operator>(const unexpected_type<std::exception_ptr>& x, const unexpected_type<std::exception_ptr>& y)
  ...
  inline BOOST_EXPECTED_CONSTEXPR bool operator<(const unexpected_type<boost::exception_ptr>& x, const unexpected_type<boost::exception_ptr>& y)
  {
    return false;
  }
  inline BOOST_EXPECTED_CONSTEXPR bool operator>(const unexpected_type<boost::exception_ptr>& x, const unexpected_type<boost::exception_ptr>& y)
