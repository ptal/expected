...
struct valued_traits<category::default_> : std::true_type {
  template <class M>
  static BOOST_EXPECTED_CONSTEXPR auto get_value(M&& m) -> decltype (has_value(m) ? deref(m) : throw valued::bad_access(), deref(m))
  {
...
{
  template <class M>
  static BOOST_EXPECTED_CONSTEXPR bool has_value(M&& m)
  { return m.has_value(); }

  template <class M>
  static BOOST_EXPECTED_CONSTEXPR auto deref(M&& m) -> decltype(m.deref())
  { return m.deref(); }
